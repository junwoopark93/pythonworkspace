...
def scrape_headline_news():
    ...
    soup = create_soup(url)
    news_list = soup.find("ul", attrs={"class":"hdline_article_list"}).find_all("li", limit=3)  # limit 을 통해 갯수 지정 가능
...
def scrape_it_news():
    ...
    for index, news in enumerate(news_list):
        ...
        img = news.find("img")
        if img:
            a_idx = 1  # img 태그가 있으면 1번째 a태그 정보 사용
